...
-- HL2 Loading Screen - uses _rt_fullframefb which contains the last rendered frame
local HL2LoadingMaterial = nil
local HL2UsingTransition = false

-- Get the last rendered frame from the game
local function GetLastFrame()
	return CreateMaterial( "hl2_last_frame_" .. SysTime(), "UnlitGeneric", {
...
function PANEL:ShowURL( url, force )
	...
	-- Skip HTML entirely if we're using the transition screen
	if HL2UsingTransition then return end
...
function PANEL:Paint()
	...
	-- Level transition: Draw captured frame with HL2-style loading panel
	if HL2UsingTransition and HL2LoadingMaterial then
		surface.SetDrawColor( 255, 255, 255, 255 )
		surface.SetMaterial( HL2LoadingMaterial )
		surface.DrawTexturedRect( 0, 0, w, h )
		
		-- Draw HL2-style loading panel in center
		...
		-- Don't draw grey background, don't run JS - pure Lua transition screen
		return
	end
	
	-- Not a transition: Draw grey background while HTML loads
...
function PANEL:RunJavascript( str )
	...
	-- Skip if using transition screen
	if HL2UsingTransition then return end
...
function PANEL:OnActivate()
	...
	-- Reset state
	HL2LoadingMaterial = nil
	HL2UsingTransition = false
	
	-- Try to get the last frame texture immediately
	HL2LoadingMaterial = GetLastFrame()
	if HL2LoadingMaterial and not HL2LoadingMaterial:IsError() then
		HL2UsingTransition = true
	else
		HL2LoadingMaterial = nil
	end
	
	-- Only load HTML if we're NOT using the transition screen
	if not HL2UsingTransition then
...
function GameDetails( servername, serverurl, mapname, maxplayers, steamid, gamemode, noDump )
	...
	-- Skip server loading URL if we're using transition screen
	if ( !HL2UsingTransition && maxplayers > 1 && GetConVar( "cl_enable_loadingurl" ):GetBool() && ( serverurl:StartsWith( "http" ) || serverurl:StartsWith( "asset://" ) ) ) then
